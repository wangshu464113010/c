 ...
 * 位运算
 * 未知数x
 * 求与x相反的数：y = ~x + 1 [x为正数,则y为负数] (x ^ -1) + 1
 * 求x是否为偶数：y = x & 1 [y=0则x为偶数]
 * 求与x%2n的余数：y = x & (2n -1) [n是整数，y = x % 2n]
 * 求x的绝对值： y = (x ^ (x >> 63)) - (x >> 63) [当x为int类型的时候,63可以换成31,(byte,short可以对应的变小)]
 * 另外一种绝对值的写法 y = x^(~(x>>63)+1)+(x>>63)
 * 两数相乘是否具有相同符号 y = (x1 ^ x2) > 0
 *
 */
